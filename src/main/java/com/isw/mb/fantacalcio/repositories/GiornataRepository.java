...
public interface GiornataRepository extends JpaRepository<Giornata, Integer> {
    ...
    @Query("SELECT g FROM Giornata g WHERE g.tsInizio <= :now AND g.tsInizio >= :nowMinus48Hours AND g.deleted = 'N'")
    ...
    Giornata findFirstByTsInizioGreaterThanAndDeleted(LocalDateTime now, char n);

    @Query("SELECT g FROM Giornata g WHERE g.deleted = 'N' ORDER BY g.numero")
    List<Giornata> findAllOrderByNumero();

    Giornata findByNumeroAndDeleted(Integer numGiornata, char n);
}